...
- **2026-07-24 — Phase 5 (initial deploy) complete.** Deployed via Vercel CLI (project `ag777/agility-scaffold-tmp`) — first deploy was auto-assigned to production by Vercel: **https://agility-scaffold-tmp.vercel.app**. All 17 routes verified live (200s on `/`, `/services`, `/book`). Git-based auto-deploy-on-push is not yet confirmed working — `vercel link` initially reported the Vercel account lacks a GitHub login connection, though `vercel git connect` later reported the repo as already connected; this commit doubles as the real-world test of whether pushing now triggers an automatic deployment.
